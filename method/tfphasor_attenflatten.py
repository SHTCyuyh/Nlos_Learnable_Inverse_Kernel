...
import pdb
class tfphasor_attenflatten(nn.Module):
    
    def __init__(self, spatial=256, crop=512, \
                 bin_len=0.01, wall_size=2.0, \
                 sampling_coeff=2.0, \
                 cycles=5, in_chans=1):
        super(tfphasor_attenflatten, self).__init__()
    ...
    def fre_atten2(self, kernel):
        ...
        # l2h = self.attention(low_part, high_part)   ##high_+ l atten
        atten_kernel = self.get_kernel(high_part,l2l,kernel)
