...
class tfpf_low_multikernel2(nn.Module):
    
    def __init__(self, spatial=256, crop=512, \
                 bin_len=0.01, wall_size=2.0, \
                 sampling_coeff=2.0, \
                 cycles=5):
        super(tfpf_low_multikernel2, self).__init__()
    ...
    def parpareparam(self):
        ...
        # wights = torch.ones((self.number_kernel))
        # # self.wights = self.wights / torch.sum(self.wights)
        
        # self.wights = nn.Parameter(wights)
        # self.wights.requires_grad = True
        ########################################################3
        ...
        learnanle_w_re = torch.zeros(self.number_kernel, temprol_grid//self.scale*2, \
                                    sptial_grid//self.scale*2, sptial_grid//self.scale*2).unsqueeze(0)
        learnanle_w_im = torch.zeros(self.number_kernel, temprol_grid//self.scale*2, \
                                    sptial_grid//self.scale*2, sptial_grid//self.scale*2).unsqueeze(0)
        learnanle_w = torch.cat([learnanle_w_re, learnanle_w_im], dim=0)
        # learnanle_w = learnanle_w.repeat(self.number_kernel, 1,1,1,1)                           
    ...
    def forward(self, feture_bxdxtxhxw):
        ...
        # refre = torch.stack([re_real, re_imag], dim=4)
        invsre = self.invpsf_real_todev
        invsim = self.invpsf_imag_todev
        ...
        B, T, H, W = invsre.shape
        tmp_re = self.learnanle_w_todev[0].reshape(B,T,H,W)
                                    
        tmp_im = self.learnanle_w_todev[1].reshape(B,T,H,W)
        for i in range(self.number_kernel):
           w1 = invsre + tmp_re
           w2 = invsim + tmp_im
        #    w1 = invsre.reshape(-1, temprol_grid*2, sptial_grid*2, sptial_grid*2)
        #    w2 = invsim.reshape(-1, temprol_grid*2, sptial_grid*2, sptial_grid*2)
        #    w1 = torch.fft.ifftshift(w1_s)
        #    w2 = torch.fft.ifftshift(w2_s)
           re_real = datafre_real * w1 - datafre_imag * w2
           re_imag = datafre_real * w2 + datafre_imag * w1
        ...
        invsre = invsre.reshape(B, T, H, W)
        invsim = invsim.reshape(B, T, H, W)
...
if __name__ == '__main__':
    ...
    if test_phasor:
        model = phasor(spatial= 64, crop=128, bin_len=0.01 * 4, \
                       wall_size=2)
