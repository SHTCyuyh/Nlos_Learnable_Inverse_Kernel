...
from positional_encodings.torch_encodings import  Summer, PositionalEncodingPermute3D, PositionalEncodingPermute1D
import pdb
class tfphasor_attenflatten_all(nn.Module):
    
    def __init__(self, spatial=256, crop=512, \
                 bin_len=0.01, wall_size=2.0, \
                 sampling_coeff=2.0, \
                 cycles=5, in_chans=1):
        super(tfphasor_attenflatten_all, self).__init__()
        ...
        # fre = spatial * spatial * crop
        self.pos_emb = Summer(PositionalEncodingPermute1D(1))
    ...
    def attention(self, kernel1, kernel2):
        ...
        if kernel1 == None: return kernel2
            ...
            kernel2_tem = kernel2[None,...].flatten(2)
            # pdb.set_trace()
            kernel2 = self.pos_emb(kernel2_tem)
            kernel2 = kernel2.view(-1,self.crop,self.spatial_grid, self.spatial_grid)
    ...
    def fre_atten2(self, kernel):
        ...
        h2h = self.attention(high_part, high_part)   ##high_+ l atten
        atten_kernel = self.get_kernel(h2h,l2l,kernel)
