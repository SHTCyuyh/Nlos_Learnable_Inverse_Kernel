...
import pdb
class tfphasor_atten_low(nn.Module):
    
    def __init__(self, spatial=256, crop=512, \
                 bin_len=0.01, wall_size=2.0, \
                 sampling_coeff=2.0, \
                 cycles=5, in_chans=1):
        super(tfphasor_atten_low, self).__init__()
    ...
    def fre_atten2(self, kernel):
        ...
        h2l = self.attention(high_part, low_part)   ##low__+ h atten
        # l2h = self.attention(low_part, high_part)   ##high_+ l atten
        atten_kernel = self.get_kernel(high_part,h2l,kernel)
...
if __name__ == '__main__':
    ...
    path2= '/data2/nlospose/chen_task/depthdataset2/train/meas/person02-00842.hdr'



    if test_phasor:
        model = tfphasor_atten(spatial= 128, crop=256, bin_len=0.01*2, \
                       wall_size=2)
