...
import pdb
class tfphasor_atten_fusion_sep(nn.Module):
    
    def __init__(self, spatial=256, crop=512, \
                 bin_len=0.01, wall_size=2.0, \
                 sampling_coeff=2.0, \
                 cycles=5, in_chans=1):
        super(tfphasor_atten_fusion_sep, self).__init__()
    ...
    def get_high_part(self, kernel, sep = 0.25):
        pdb.set_trace()
        T,H,W  = kernel.squeeze().shape
        mask = torch.zeros_like(kernel)
        center_t, center_h, center_w = T//2, H//2, W//2
        size_t, size_h, size_w = (T*sep) // 2, (T*sep) // 2, (T*sep) // 2
        mask[:,center_t-size_t:center_t+size_t,center_h-size_h:center_h+size_h,center_w-size_w:center_w+size_w] += 1 
        mask = mask.to(kernel.device)
        high_part = kernel * mask
        return high_part
...
if __name__ == '__main__':
    ...
    if test_phasor:
        model = tfphasor_atten_fusion_sep(spatial= 128, crop=256, bin_len=0.01*2, \
                       wall_size=2)
        model.todev(dev='cuda:0',dnum=4)
