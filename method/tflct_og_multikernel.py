...
class tflct_og_multikernel(nn.Module):
    ...
    def parpareparam(self):
        ...
        self.wights = torch.ones((self.number_kernel))
        # self.wights = self.wights / torch.sum(self.wights)
        self.wights.requires_grad = True
        self.wights = nn.Parameter(self.wights)
        ...
        # learnanle_w = learnanle_w.repeat(self.number_kernel, 1,1,1,1)  #n 2 t s s
    ...
    def forward(self, feture_bxdxtxhxw):
        ...
        #    w1_s = self.pad(self.learnanle_w_todev[i,0].unsqueeze(0)) + invsre
        #    w2_s = self.pad(self.learnanle_w_todev[i,1].unsqueeze(0)) + invsim
           ...
           volumn_BDxTxHxW = volumn_BDxTxHxW.reshape(-1, self.number_kernel, temprol_grid//self.scale, \
                                                    sptial_grid//self.scale, sptial_grid//self.scale)
           results[:,i] =  self.wights[i] * volumn_BDxTxHxW[:,i]
...
if __name__ == '__main__':
    ...
    if test_lct:
        model = tflct_og_multikernel(spatial= 128, crop=256, bin_len=0.01 * 2, \
