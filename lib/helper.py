...
def waveconvparam(bin_resolution, virtual_wavelength, cycles):
    ...
    grids_k = np.arange(samples, dtype=np.float32) + 1  #??grids_k ???
...
def waveconv(bin_resolution, virtual_wavelength, cycles, data_txhxw):
    ...
    c = 3e8;
    s_z = bin_resolution * c;
    samples = int(round(cycles * virtual_wavelength / (bin_resolution)));
    num_cycles = samples * bin_resolution / virtual_wavelength;
    sigma = 0.3;
    ...
    return wave_cos, wave_sin, virtual_sin_wave_k, virtual_cos_wave_k
